...
一個宣稱在做誠實研究的 repo，應該對自己也誠實。

1. **`_compute_daily_adx` 有前瞻偏誤。** D 日的 ADX 用 D 日全天的 high/low 計算，
   但策略在 09:30 就讀它，那時當日盤還沒走完。ADX 濾網預設停用
   （`adx_period=0`），無 live 策略受影響。以 `xfail` 測試釘住，修法是一行
   `.shift(1)`。附帶一提：先前研究發現 ADX 在此無預測力——一個能偷看未來的濾網
   仍然測不出 edge，反而讓那個結論更可信。
2. **`trend_ma_days` 的單位不是「天」。** `runner.py` 用
   `n_bars = trend_ma_days * 301`，但 301 是**日盤** bar 數，卻套用在含夜盤的序列上
   （1,142 bar/交易日）。所以 `trend_ma_days=10` 實際回看約 2.6 個交易日。策略內的
   fallback 路徑用同一個常數卻是**正確的**，於是同一個參數依載入方式不同而語意不同。
   該均線仍是 trailing，沒有前瞻，**既有結果依然有效**——錯的是標籤。
3. **Python ↔ Pine Script 的等價性未經驗證。** Confirmed 策略會重寫成 TradingView
   指標實際執行，而那個語言跑不了這套回測，也沒有任何東西在檢查兩邊一致。
   第 2 點正是最容易在轉譯時出事的那種。
4. **任何回測的第一個交易日都不會產生交易**——開盤區間指標在 09:30 前是 `NaN`，
   引擎會跳過所有指標尚未就緒的前導 bar。回測區間短時，這會安靜地吃掉可觀的樣本。
5. **`end="YYYY-MM-DD"` 會整天排除該日**——它被解析成當天 00:00，而日盤 08:45 才開始。
6. **測試覆蓋是針對性的，不是全面的。** 特徵層與一支策略的規則有覆蓋；其餘策略、
   選擇權回測與探索腳本沒有。

第 4、5 點已由測試釘住，不會靜默回歸。
