...
寫上面那些測試時，抓到一個已經影響結果好幾個月的 bug。

**`trend_ma_days` 的單位不是「天」。** 趨勢均線的窗口是 `trend_ma_days * 301` 根 bar，
而 301 是**日盤**的 bar 數（[`runner.py:189`](src/backtest/runner.py#L189)，
[`runner.py:348`](src/backtest/runner.py#L348) 另有一處硬編碼）。兩處都套用在含夜盤的
連續序列上，而那是 1142 bar/交易日。所以 `trend_ma_days=10`——
[`orb.py:214`](src/strategies/orb.py#L214) 的策略說明寫著「10 日趨勢均線」——
實際只回看約 2.6 個交易日。同一個 `× 301` 在 `orb.py:51` 與 `orb.py:324` 是**正確的**，
因為那裡的序列真的只有日盤。一個參數兩種意義，取決於哪個 loader 跑過。

後果不是「回測無效」——均線仍然是 trailing，沒有前瞻。真正的問題是
`trend_ma_days=10` 這個「最佳參數」，是**在 2.6 日的版本上**優化出來的。
把常數改對並不會把參數改對，整組搜尋得重跑。

另有一個潛在缺陷：`_compute_daily_adx` 沒有 shift，D 日的 ADX 用到 D 日自己的高低點。
但 `adx_period` 預設為 `0`，濾網一直是關的，沒有任何策略讀過它。

兩個都還沒修，因為第一個會讓「選出現行參數」的那些回測失效。目前用
`xfail(strict=True)` 測試釘住**應有行為**：誰修好了，測試就會開始通過，strict 模式
讓 build 失敗，那正是「回頭重做所有下游」的提醒。完整說明在
[`tests/test_lookahead.py`](tests/test_lookahead.py)。

最大的未測試面在另一個地方：Confirmed 策略會重寫成 TradingView Pine Script——
那才是實際下單的語言，而它跑不了這套回測。沒有任何東西在檢查兩邊的實作是否一致。
