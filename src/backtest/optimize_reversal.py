...
def main():
    ...
    total_combos = (len(GRID["vol_ratio"])
                    * len(GRID["sl_ema_fraction"])
                    * len(GRID["tp_ema_fraction"]))
    ...
    for vol, sl, tp in product(
        GRID["vol_ratio"], GRID["sl_ema_fraction"], GRID["tp_ema_fraction"]
    ):
        params = dict(vol_ratio=vol, sl_ema_fraction=sl, tp_ema_fraction=tp)
        ...
        print(f"  [{done:2d}/{total_combos}] vol={vol:.1f} sl={sl:.2f} tp={tp:.1f}"
              f"  total={tot_row['total']:+.0f}", flush=True)

    df_s = pd.DataFrame(summary)
    yr_hdr = "  ".join(f"{'y'+y:>7}" for y in yr_cols)
    print(f"\n  {'vol':>5}  {'sl':>5}  {'tp':>5}  {yr_hdr}  {'TOTAL':>8}")
    print(f"  {'-'*5}  {'-'*5}  {'-'*5}  "
    ...
    for _, r in df_s.sort_values("total", ascending=False).iterrows():
        ...
        print(f"  {r['vol']:5.1f}  {r['sl']:5.2f}  {r['tp']:5.1f}  {yv}  "
              f"{fv(r['total'], 8, 0):>8}")
