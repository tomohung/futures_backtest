#!/usr/bin/env python3
"""Phase 4 optimizer: adaptive TP using OR width.

  TP = entry ± tp_or_multiplier × max(OR_width, or_min_width)
  SL = sl_pct from entry  (same as Phase 2)
  Trailing = sl_pct trailing after trail_activate_minute  (same as Phase 2)

Grid: tp_or_multiplier × sl_pct  (7 × 3 = 21 combos)

Additional: historical year sweep (2021–2026) for best combo validation.
Exit breakdown added: tp_exit%, sl_exit%, force_exit%.

Usage:
    uv run python src/backtest/optimize_phase4.py
"""
import sys
...
from src.strategies.orb import ORBPhase4Strategy, ORBStrategy
...
def compute_metrics(stats) -> dict | None:
    ...
    # Exit type breakdown
    tp_mask = ~force_mask & (pnl > 0)   # intraday profitable exits
    sl_mask = ~force_mask & (pnl <= 0)  # intraday loss exits
    n = len(trades)
...
def run_grid(df, combos: list[dict], label: str) -> pd.DataFrame:
    ...
    for i, params in enumerate(combos, 1):
        m = run_single(df, ORBPhase4Strategy, params)
        if m:
...
def main():
    ...
    print("=" * 70)
    print("Phase 4 — Adaptive TP (OR-width based)")
    print("  TP = entry ± tp_or_multiplier × max(OR_width, or_min_width=20)")
    ...
    merged.to_csv("output/phase4_results.csv", index=False)
    print("\nResults → output/phase4_results.csv")
    ...
    print(f"\n{'='*70}")
    print("HISTORICAL YEAR SWEEP — best Phase 4 params")
    print(f"  Params: {best_label}")
    print(f"{'='*70}")

    print("\nLoading all-years data...", flush=True)
    df_all = load_data_with_night_ma(trend_ma_days=10)

    total_p2 = total_p4 = 0.0
    hdr = (f"  {'Year':<6}  {'Ph2 exp':>8}  {'Ph2 PF':>7}  {'Ph2 tot':>8}  {'Ph2 force%':>10}"
           f"  {'Ph4 exp':>8}  {'Ph4 PF':>7}  {'Ph4 tot':>8}  {'Ph4 tp%':>8}  {'Ph4 force%':>10}")
    print(hdr)
    print("  " + "-" * (len(hdr) - 2))
    ...
    for yr, start, end in YEARS:
        ...
        m4 = run_single(df_yr, ORBPhase4Strategy, best_params)
        ...
        print(f"  {yr:<6}  {fv(m2.get('expectancy') if m2 else None):>8}"
              f"  {fv(m2.get('pf') if m2 else None):>7}"
              f"  {fv(t2):>8}"
              f"  {fv(m2.get('force_pct') if m2 else None):>9}%"
              f"  {fv(m4.get('expectancy') if m4 else None):>8}"
              f"  {fv(m4.get('pf') if m4 else None):>7}"
              f"  {fv(t4):>8}"
              f"  {fv(m4.get('tp_exit') if m4 else None):>7}%"
              f"  {fv(m4.get('force_pct') if m4 else None):>9}%")

    print(f"  {'TOTAL':<6}  {'':>8}  {'':>7}  {fv(total_p2):>8}"
          f"  {'':>10}  {'':>8}  {'':>7}  {fv(total_p4):>8}")

    # ── Detailed comparison on Train+OOS ──────────────────────────────────
    print(f"\n{'='*70}")
    print("FINAL COMPARISON — Phase 2 vs Phase 4 (best)")
    print(f"  Phase 4 params: {best_label}")
    print(f"{'='*70}")

    for period, df_p in [("Train 2025", df_train), ("OOS 2026", df_test)]:
        m2 = run_single(df_p, ORBStrategy,      PHASE2_BASE) or {}
        m4 = run_single(df_p, ORBPhase4Strategy, best_params) or {}

        print(f"\n  Period: {period}")
        print(f"  {'Metric':<22}  {'Phase2 Base':>14}  {'Phase4 best':>14}")
        print(f"  {'-'*22}  {'-'*14}  {'-'*14}")
        ...
        rows = [
            ...
            ("win_rate",     fv(m2.get("win_rate")),    fv(m4.get("win_rate"))),
            ("avg_wl",       fv(m2.get("avg_wl")),      fv(m4.get("avg_wl"))),
            ("pf",           fv(m2.get("pf")),          fv(m4.get("pf"))),
            ("expectancy",   fv(m2.get("expectancy")),  fv(m4.get("expectancy"))),
            ("total PnL",    fv(m2.get("total")),       fv(m4.get("total"))),
            ("tp_exit %",    fv(m2.get("tp_exit")),     fv(m4.get("tp_exit"))),
            ("sl_exit %",    fv(m2.get("sl_exit")),     fv(m4.get("sl_exit"))),
            ("force_exit %", fv(m2.get("force_pct")),   fv(m4.get("force_pct"))),
        ]
        for lbl, b, h in rows:
            print(f"  {lbl:<22}  {b:>14}  {h:>14}")

        for direction in ("long", "short"):
            bd = m2.get(direction) or {}
            hd = m4.get(direction) or {}
            print(f"\n    [{direction.upper()}]")
            print(f"    {'Metric':<20}  {'Phase2 Base':>14}  {'Phase4 best':>14}")
            print(f"    {'-'*20}  {'-'*14}  {'-'*14}")
            for sub in ["n", "win_rate", "avg_wl", "pf", "expectancy"]:
                print(f"    {sub:<20}  {fv(bd.get(sub)):>14}  {fv(hd.get(sub)):>14}")
