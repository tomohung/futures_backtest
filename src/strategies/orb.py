...
class ORBLongStrategy(Strategy):
    ...
    ⚠️ 上列為回測模擬結果，**未計手續費、交易稅與滑價**，且參數是在同一段
       歷史上優化出來的（見 research/archive/confirmed/H026-orb-longonly 等）。
       這些數字的用途是「跨參數／跨版本比較」，不是可實現報酬的估計。
       另注意 trend_ma_days 的實際回看期間與字面天數不同，見該參數註解。
       免責聲明見 LICENSE。
    ════════════════════════════════════════════════════════════
    """
    ...
    # ── 趨勢濾網 ────────────────────────────────────────────────────────
    # ⚠️ 這個「天數」與實際回看期間不相等，移植到 Pine Script 前務必看完：
    #
    #   走 TrendMA 欄位時（load_data_with_night_ma 提供，正式回測都走這條）：
    #     runner.py 用 n_bars = trend_ma_days × 301，但 301 是**日盤** bar 數，
    #     而該均線算在含夜盤的連續序列上（1142 bar/交易日）。
    #     → trend_ma_days=10 的實際回看 ≈ 10 × 301 / 1142 ≈ 2.6 個交易日。
    #
    #   走本檔 init() 的 fallback 時（self.data.Close 只有日盤）：
    #     同一個 × 301 是正確的 → 實際回看就是 10 個交易日。
    #
    # 也就是同一個參數在兩條路徑下語意不同。現行「最佳參數 10」是在前者
    # （≈2.6 日）下優化出來的，改成後者等於換了一個策略。
    # 轉 Pine Script 時請換算成等效 bar 數，不要直接寫日線 SMA(10)。
    # 兩條路徑的行為差異由 tests/test_lookahead.py 釘住（含一條 xfail 標記
    # 應有行為，修好會自動提醒）。
    trend_ma_days: int = 10         # 趨勢均線回溯天數（見上方警告：非字面天數）
